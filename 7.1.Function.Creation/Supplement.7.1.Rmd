...
To a function name, you assign the output of the `function()` function. You can provide arguments, write the necessary code to carry out the task that the function will perform, and then return the value the function produces using the `return()` function.
...
Let's practice writing functions by creating another function. This function will be called "is_RNA_seq" and it will check if a character value "seq" is an RNA sequence. 
...
Along with adding warning messages to our functions we can also add *error messages*. 
A error is thrown with the `stop()` function. It takes an error message as an argument. Unlike a warning, an error DOES stop the rest of the function from running.
