  ...
  cases = list(
    ottr::TestCase$new(
      ...
      code = {
        ...
        expect_true(any(grepl("age", pancreatic_subtypes_age_mutations_survival$mapping$x)))
        expect_true(any(grepl("mutational_burden", pancreatic_subtypes_age_mutations_survival$mapping$y)))
        expect_true(any(grepl("cancer_subtype", pancreatic_subtypes_age_mutations_survival$mapping$colour)) | 
                      any(grepl("colour", names(pancreatic_subtypes_age_mutations_survival$layers[[1]]$mapping))))
      }
    )
  )
)